...
const INFO_TEXT = utils.Convert.FromBinary("InfoText");
const INFO_RATCHET = utils.Convert.FromBinary("InfoRatchet");
const INFO_MESSAGE_KEYS = utils.Convert.FromBinary("InfoMessageKeys");
...
class Secret {
    ...
    static async HKDF(IKM, keysCount = 1, salt, info = new ArrayBuffer(0)) {
        ...
        for (let i = 0; i < keysCount; i++) {
            T[i + 1] = await this.sign(PRK, utils.combine(T[i], info, new Uint8Array([i + 1]).buffer));
        }
        return T.slice(1);
    }
}
...
class ECPublicKey {
    static async create(publicKey) {
        ...
        const x = utils.Convert.FromBase64Url(jwk.x);
        const y = utils.Convert.FromBase64Url(jwk.y);
        const xy = utils.Convert.ToBinary(x) + utils.Convert.ToBinary(y);
        res.serialized = utils.Convert.FromBinary(xy);
        res.id = await res.thumbprint();
        return res;
    }
    static async importKey(bytes, type) {
        const x = utils.Convert.ToBase64Url(bytes.slice(0, 32));
        const y = utils.Convert.ToBase64Url(bytes.slice(32));
        const jwk = {
    ...
    async thumbprint() {
        ...
        return utils.Convert.ToHex(thumbprint);
    }
    async isEqual(other) {
        if (!(other && other instanceof ECPublicKey)) {
            return false;
        }
        return utils.isEqual(this.serialized, other.serialized);
    }
}
...
class DateConverter {
    static async set(value) {
        return new Uint8Array(utils.Convert.FromString(value.toISOString()));
    }
    static async get(value) {
        return new Date(utils.Convert.ToString(value));
    }
}
...
let IdentityProtocol = (() => {
    ...
    IdentityProtocol = IdentityProtocol_1 = tslib.__decorate([
...
let MessageSignedProtocol = (() => {
    ...
    MessageSignedProtocol = tslib.__decorate([
...
let PreKeyMessageProtocol = (() => {
    ...
    tslib.__decorate([
        tsprotobuf.ProtobufProperty({ id: 5, parser: IdentityProtocol, required: true })
    ], PreKeyMessageProtocol.prototype, "identity", void 0);
    tslib.__decorate([
        tsprotobuf.ProtobufProperty({ id: 6, parser: MessageSignedProtocol, required: true })
    ], PreKeyMessageProtocol.prototype, "signedMessage", void 0);
    PreKeyMessageProtocol = tslib.__decorate([
...
let PreKeyBundleProtocol = (() => {
    ...
    tslib.__decorate([
        tsprotobuf.ProtobufProperty({ id: 2, parser: IdentityProtocol, required: true })
    ...
    PreKeyBundleProtocol = tslib.__decorate([
...
exports.IdentityProtocol = IdentityProtocol;
exports.MessageSignedProtocol = MessageSignedProtocol;
exports.PreKeyBundleProtocol = PreKeyBundleProtocol;
exports.PreKeyMessageProtocol = PreKeyMessageProtocol;
